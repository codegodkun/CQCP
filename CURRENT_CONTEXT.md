...
- `TASK-036` 的 `TASK_SPEC-036-A` 未激活 carrier foundation 已随 PR #32 合并。`TASK_SPEC-036-B1` 已冻结 `v20260715.1` 静态 consistency policy、九点显式双预算和未激活校验；首轮规格审计 findings 已整改，第二轮独立复审 `GO`。当前仅允许 CC 提交编码前规格映射计划，Codex 放行前无代码授权；现有任务仍走 legacy 单 anchor。
...
4. `TASK_SPEC-036-B1` 已冻结并通过独立规格审计 `GO`；下一步为 CC 编码前规格映射计划。B1 接纳后再冻结 B2，随后才进入 C。

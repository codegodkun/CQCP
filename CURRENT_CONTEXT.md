...
- `TASK-036` 的 `TASK_SPEC-036-A` 未激活 carrier foundation 已随 PR #32 合并。`TASK_SPEC-036-B1` 五路径实现已通过 Codex 接纳与第二轮独立实现审计 `GO_TO_ACCEPT`，当前等待精确 commit。`TASK_SPEC-036-B2` runtime loader / fail-closed activation gate foundation 已创建、冻结并在首轮 `NO_GO` 整改后通过增量独立规格审计 `GO`；Codex Decision 为 `SPEC_FROZEN / CODE_BLOCKED_BY_B1_COMMIT / NO_IMPLEMENTATION_AUTHORIZATION`。新 `v20260715.1` 仍为 `DRAFT / NOT_BOUND / false/false/NONE`，现有任务继续走 legacy single anchor；C 与正式 MVP E2E 均未解锁。
...
- `TASK_SPEC-035-A` 已实现、接纳并提交；candidate comparison 路线不再阻塞。`TASK-036-A` 未激活 carrier 已接纳，B1 已接纳但未提交，B2 仅完成 loader/gate 规格冻结且编码被 B1 commit 门禁阻断，C 的真实 scope collector 与可靠异值裁判仍未冻结，因此 `TASK-034` 正式重跑继续被阻塞；不得提前改写 v1 的 9 `MATCH` / 18 `MISMATCH` 与 57/57 `NOT_OBSERVABLE` 证据。
...
- 不进入真实 DOCX `TABLE_CELL` coverage 激活、RuleSetVersion readiness 或 B2/C 批次实现；B2 当前只完成规格冻结与审计。
...
- 待确认：是否精确 commit 已接纳的 TASK_SPEC-036-B1 五路径实现与本轮治理文档。B2 已冻结并审计 `GO`，但 B1 commit 前编码阻断；C 仍须另行冻结和独立审计。
...
4. `TASK_SPEC-036-B1` 已实现、接纳并等待精确 commit；`TASK_SPEC-036-B2` 已冻结并通过独立规格审计 `GO`。B1 形成可重建 commit 后，下一步才是 B2 编码前规格映射计划与 Codex 单独 Review；随后才可另行冻结 C。

...
- `TASK-033` 已建档为 active 规格冻结任务，文件：`tasks/active/TASK-033-mvp-e2e-sample-acceptance-spec-freeze.md`。本任务只冻结 MVP 端到端样本验收规格、输入字段、验收命令、证据口径和 expected 来源说明；不运行完整验收，不修改代码、测试、fixture、expected JSON、parser、`CandidateResolver`、`EvidenceSlot` 或 `SourceAnchor`；正式提交前需要独立 agent 只读复核。
...
- 已确认：`TASK-033` 已正式建档为 MVP 端到端样本验收规格冻结任务；提交前需独立只读复核 expected 来源、证据口径和验收指标。
...
5. `TASK-033` 下一步是正式提交前独立 agent 只读复核；复核通过前不得提交、归档或进入实现任务。

...
- `TASK-035` 的 `mvp-e2e-candidate-comparison-v2` test-only 比较契约与 `TASK_SPEC-035-A` 实现均已接纳。唯一 harness 实现提交 `52d73b3` 已随 PR #32 合并；定向复验为 harness `15/15`、四类回归 `27/27`。未运行正式 MVP E2E，父任务仍 active。
- `TASK-036` 的 `TASK_SPEC-036-A` 未激活 carrier foundation 已随 PR #32 合并。`TASK_SPEC-036-B1` 已通过提交 `137c202` 形成可重建静态 policy 基线。`TASK_SPEC-036-B2` runtime loader / fail-closed activation gate foundation 已实现，通过 Codex Review、最终独立实现审计 `GO`、loader/gate 209/209 与真实 Linux Docker image build；Codex Decision 为 `ACCEPT_IMPLEMENTATION / B2_FOUNDATION_ONLY / AWAITING_PRECISE_COMMIT_AUTHORIZATION`。新 `v20260715.1` 仍为 `DRAFT / NOT_BOUND / false/false/NONE`，现有任务继续走 legacy single anchor；C 与正式 MVP E2E 均未解锁。
...
- PR #32：`TASK-034` 后续门禁、`TASK-035` candidate comparison v2 与 `TASK_SPEC-036-A` 未激活 carrier foundation，merge commit `97ef08f1cae88e8a702069eb0e07c2035b3b063f`，head commit `c2fd17e7af888d0bace21004065475cc36d13469`。三项 checks 均为 `SUCCESS`。合并不激活 RuleSetVersion runtime loader，不声明 57/57 occurrence coverage，不授权正式 MVP E2E，也不进入 `TASK-028` / `TASK-031` / `TASK-032`。
...
- `TASK_SPEC-035-A` 已实现、接纳并提交；candidate comparison 路线不再阻塞。`TASK-036-A` 未激活 carrier 已接纳，B1 已提交，B2 loader/gate foundation 已接纳但尚未提交；C 的真实 scope collector 与可靠异值裁判仍未冻结，因此 `TASK-034` 正式重跑继续被阻塞；不得提前改写 v1 的 9 `MATCH` / 18 `MISMATCH` 与 57/57 `NOT_OBSERVABLE` 证据。
- `TASK-036-A` 已接纳但仅为未激活 carrier，不改变当前规则集 anchor 基数；B2 明确在 C 前 fail-closed，不把 `v20260715.1` 写为已生效 execution 版本，因此正式重跑仍被阻塞。
...
- 不进入真实 DOCX `TABLE_CELL` coverage 激活、RuleSetVersion readiness 或 C 批次实现；B2 仅接纳 loader/gate foundation，尚未生产激活。
...
- `TASK_SPEC-036-A` 已接纳并获得本轮精确提交授权；不得修改接纳后的七文件 diff，除非新 finding 经 Codex 明确定界。A 永不修改 EvidenceSlot、结果快照合同、现有 preparer、OpenAPI、数据库、parser、fixture、expected JSON、Docker、workflow、已接受 ADR 或 PRD。
...
- 已确认：用户授权进入 TASK-036 生产实现治理流程；`TASK_SPEC-036-A` 已冻结、实现、通过独立规格与实现审计并经 Codex 接纳。该接纳不自动授权 commit、B/C 或正式 E2E。
- 已确认：`TASK_SPEC-035-A` 编码前计划、实现、Codex Review Intake、独立只读实现审计和精确提交均已完成；提交为 `52d73b3`。
- 待确认：是否精确 commit 已接纳的 TASK_SPEC-036-B2 十路径实现与本轮治理写回；B1 已提交为 `137c202`，C 仍须另行冻结和独立审计。
...
3. `TASK_SPEC-035-A` 已实现并接纳，提交 `52d73b3` 已随 PR #32 合并；保持 `TASK-034` v1 正式证据不变，不执行正式重跑。
4. `TASK_SPEC-036-B1` 已提交为 `137c202`；`TASK_SPEC-036-B2` 已实现并通过 Codex、最终独立审计与真实 Linux Docker build，当前等待精确 commit 授权。B2 提交形成可重建基线后，才可另行创建和冻结 C；正式 E2E 仍需单独授权。
5. `TASK-GOV-006` 已通过 PR #18 完成云端 PR 触发验证；该结果仍不得用于进入业务实现或配置 required status checks。
6. `TASK-033` 已归档；`TASK-DATA-001` 人工 anchor 标注及 fixture / expected JSON 引用 / 定向测试转换已经完成；正式 MVP 端到端验收由 `TASK-034` 承接。该任务不自动解除 required status checks 或 `TASK-028` / `TASK-031` / `TASK-032` 的各自门禁。
7. `TASK-030` 已归档；runtime loader 评审和 model / budget profile 源定义仍只是候选建议，不代表已授权创建或派发下一实现任务。

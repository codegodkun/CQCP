...
- `TASK-035` 已冻结 `mvp-e2e-candidate-comparison-v2` test-only 比较契约，文件：`tasks/active/TASK-035-mvp-e2e-candidate-comparison-contract-rebaseline.md`。用户已授权创建并派发 `TASK_SPEC-035-A`；规格文件为 `tasks/active/TASK_SPEC-035-A-test-only-candidate-comparison-v2.md`，首轮规格审计两项阻断已整改，复审 `GO`。当前状态为 `SPEC_FROZEN / READY_FOR_CODING_PLAN_SUBMISSION / NO_CODE_AUTHORIZATION`，只允许 Claude Code / DeepSeek 提交编码前规格映射计划，Codex 未放行前不得编码或正式重跑。
- `TASK-036` 已建立多出处一致性证据架构冻结父任务。用户已明确接受 `ADR-016`，`docs/ARCHITECTURE.md` 已同步为 v0.10；接受后同步独立审计 `GO`，Codex Review Intake 为 `ADR_ACCEPTED / ARCHITECTURE_SYNCHRONIZED / NO_IMPLEMENTATION_AUTHORIZATION`。现有 point result / snapshot / OpenAPI / JSONB query 已只读确认可按列表承载一点评多 anchors；当前没有 CandidateResolver / EvidenceSlot / Review Engine / SourceAnchor 生产实现授权。
...
- `TASK_SPEC-035-A` 已冻结并通过独立审计，当前阻塞于 Claude Code / DeepSeek 尚未提交编码前规格映射计划及 Codex 尚未放行；不得修改人工 raw expected、用 actual 倒填 projection 或提前把 18 个差异改写为 `MATCH`。
- `TASK-036` / `ADR-016` 的架构冻结已完成，但生产实现仍未获授权；当前生产链路的 occurrence-insensitive dedup + selected-candidate 投影导致多出处 provenance 丢失，单纯补 cellIndex 不能解决。
...
- 不实现 `TASK-035` comparison v2，直到 Claude Code / DeepSeek 编码前规格映射计划经 Codex 明确 `GO / IMPLEMENTATION AUTHORIZED`。
- 不修改 CandidateResolver、EvidenceSlot、Review Engine、SourceAnchor 或结果快照；`ADR-016` 接受和 ARCHITECTURE 同步不构成生产实现授权。
- 不修改生产代码、测试、fixture、expected JSON、OpenAPI、数据库、Docker、workflow、已接受 ADR 或 PRD。
...
- 已确认：用户授权创建并派发 `TASK_SPEC-035-A`；用户接受 `ADR-016`，并要求先同步 ARCHITECTURE、不得直接生产实现。
- 待确认：Codex 是否接受 Claude Code / DeepSeek 的 TASK_SPEC-035-A 编码前规格映射计划；TASK-036 后续生产任务是否另行授权。
...
3. `TASK_SPEC-035-A` 已冻结并通过独立审计；下一步仅派发 Claude Code / DeepSeek 编码前规格映射计划，Codex 审查前不得编码。
4. `ADR-016` 已接受且 `docs/ARCHITECTURE.md` v0.10 已同步、审计 `GO`；当前停止在架构冻结，不直接进入生产实现。

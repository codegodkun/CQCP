...
- `TASK_SPEC-035-A` 已实现、接纳并提交；candidate comparison 路线不再阻塞。`TASK-034` 正式重跑仍被 `TASK-036` 未授权生产实现及其自身门禁阻塞，不得据 test-only 实现提前改写 v1 的 9 `MATCH` / 18 `MISMATCH` 证据。
...
- 待确认：TASK-036 后续生产任务是否另行授权；当前不得因 ADR-016 已接受而直接进入实现。

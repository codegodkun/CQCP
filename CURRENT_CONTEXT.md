...
- `TASK-035` 的 `mvp-e2e-candidate-comparison-v2` test-only 比较契约与 `TASK_SPEC-035-A` 实现均已接纳。编码前规格映射计划经 Codex 放行后，Claude Code / DeepSeek 仅修改既有 harness；Codex 定向复验为 harness `15/15`、四类回归 `27/27`，独立只读实现审计 `GO`、无 findings，Codex Review Intake 为 `ACCEPT_IMPLEMENTATION`。唯一 harness 实现提交为 `52d73b3`；未 push、未 merge、未运行正式 MVP E2E，父任务仍 active。
- `TASK-036` 的 `TASK_SPEC-036-A` 未激活 carrier foundation 已实现并接纳。规格独立审计最终 `GO`，编码前计划获 Codex GO；实现精确修改 2 个生产文件和 5 个测试文件。Codex 复验第一组 47/47、第二组 25/25，独立实现审计最终 `GO`，Review Intake 为 `ACCEPT_IMPLEMENTATION`。用户已授权精确提交 7 个实现文件和 5 个治理文档；现有 preparer / RuleSetVersion / 普通任务仍走 legacy 单 anchor，B/C 未授权。
...
3. `TASK_SPEC-035-A` 已实现并接纳，提交 `52d73b3`；保持 `TASK-034` v1 正式证据不变，不执行正式重跑。
4. `TASK_SPEC-036-A` 已实现并接纳、独立实现审计 `GO`，用户已授权精确提交本轮 12 个文件；提交完成后不得自动进入 B/C，也不重跑正式 E2E。

...
3. `TASK-DATA-001` 已完成规则冻结、3 份候选选择、63 条 `ACCEPTED_HUMAN_GROUND_TRUTH`、可重建提交和隔离 worktree 准备。下一步只要求 Claude Code / DeepSeek 输出 `TASK_SPEC-DATA-001-A` §0.2 编码前规格映射计划，Codex 审查计划并给出 `GO` / `NO-GO`；明确 `GO` 前不得实现。

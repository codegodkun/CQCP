...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备，三份合同共 63 条逐出处明细已由 `ZK` 接受。`TASK_SPEC-DATA-001-A` 首轮实现已回传：8 个路径均在范围内，当前 fixture / expected 内容正确，10/10 与 4/4 定向测试通过，独立 agent 对范围、独立性和 baseline 给出 `GO`；但新测试未完整锁定 JSON 类型、occurrence 顺序、全部 required columns、精确 18-key schema 和完整 positive-case baseline。Codex Decision 为 `NO-GO / TEST CONTRACT REVISION REQUIRED / DATA ARTIFACTS RETAINED / NO COMMIT OR PUSH`。`TASK-EVAL-001` 不归档，不进入 `TASK-028` / `TASK-031` / `TASK-032`。
...
- 已确认：`TASK-DATA-001` 选定的 `CQCP-MVP-DOCX-001` / `002` / `003` 可用于项目验证，脱敏合规，data owner / 确认人为 `ZK`；人工标注使用 XLSX；三份合同 63 条逐出处明细已提升为 `ACCEPTED_HUMAN_GROUND_TRUTH`；`TASK_SPEC-DATA-001-A` 当前数据产物可保留。待确认：test-only revision 和 Codex 再审。
...
3. `TASK-DATA-001` 已完成规则冻结和 63 条 ground truth；`TASK_SPEC-DATA-001-A` 下一步仅由 Claude Code / DeepSeek 修订 `HumanAnchorGroundTruthFixtureTest.java` 的四项契约断言，其他 7 个实现路径不得修改，禁止 commit 或 push。

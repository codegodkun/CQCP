...
- `TASK-036` 的 `TASK_SPEC-036-A` 未激活 carrier foundation 已实现并接纳。实现提交 `c2fd17e` 已随 PR #32 合并；第一组 47/47、第二组 25/25，独立实现审计最终 `GO`。现有 preparer / RuleSetVersion / 普通任务仍走 legacy 单 anchor；后续按 B1 静态不可变策略、B2 runtime binding/activation、C 真实 collector 与裁判集成推进。
...
- `TASK_SPEC-035-A` 已实现、接纳并提交；candidate comparison 路线不再阻塞。`TASK-036-A` 未激活 carrier 已接纳，但 B/C 的版本激活、真实 scope collector 与可靠异值裁判仍未冻结，因此 `TASK-034` 正式重跑继续被阻塞；不得提前改写 v1 的 9 `MATCH` / 18 `MISMATCH` 与 57/57 `NOT_OBSERVABLE` 证据。
- `TASK-036-A` 已接纳但仅为未激活 carrier，不改变当前规则集 anchor 基数；B 的 RuleSetVersion activation/readiness 与 C 的真实 collector/verdict 尚未冻结，因此正式重跑仍被阻塞。
...
- 不进入真实 DOCX `TABLE_CELL` coverage 激活、RuleSetVersion readiness 或 B/C 批次实现。
...
- 待确认：是否精确 commit 已接纳的 TASK_SPEC-036-A 七文件实现与本轮治理文档；是否另行授权冻结 TASK_SPEC-036-B。B/C 仍须分别走独立门禁。
...
4. `TASK_SPEC-036-A` 已通过 PR #32 合并；下一步先冻结并实施 B1 静态不可变策略与发布前校验，再推进 B2 runtime binding/activation，随后才进入 C；各批均须独立规格审计、CC 编码前计划、Codex 放行和实现复核。

...
- `TASK-GOV-006` 本地准备中，文件：`tasks/active/TASK-GOV-006-submit-authorization-evidence-gate.md`。本任务准备提交前授权证据模板、PR 模板和 `Authorization evidence check` PR body 文本门禁；本轮本地正向 fixture 已通过、反向 fixture 已按预期失败，云端 GitHub Actions 验证待真实 PR 触发；不配置 branch protection、repository ruleset 或 required status checks，不发布 `CQCP Code Review` / `CQCP Spec & Docs Review` Check Run 或 Commit Status。
...
- 已确认：建立提交前授权证据模板 / GitHub Actions PR body 文本检查 / PR 模板等机制化治理任务，当前由 `TASK-GOV-006` 承接；模板、PR body 文本检查和项目记忆写回本地准备中，云端 PR 触发验证仍待后续 PR。

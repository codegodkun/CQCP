...
- `TASK-030` 已完成 Review assets 版本化治理只读盘点，文件：`tasks/active/TASK-030-review-assets-versioning-governance.md`。`TASK_SPEC-030-A` 已完成首批静态资产源文件与 RuleSetVersion manifest 最小建档，文件：`tasks/active/TASK_SPEC-030-A-review-assets-static-source-manifest.md`。`TASK_SPEC-030-B` 已完成 schema 与 manifest 引用校验脚本 `scripts/validate-review-assets.mjs`，文件：`tasks/active/TASK_SPEC-030-B-review-assets-schema-manifest-validation.md`；当前 7 个资产 JSON 全部通过校验。`TASK_SPEC-030-C` 已完成版本治理与变更流程最小建档（README 版本命名规则、新增/升级流程、manifest 引用规则），文件：`tasks/active/TASK_SPEC-030-C-review-assets-version-governance-docs.md`。新增资产位于 `packages/review-assets/` 下，覆盖 rule set、review point definition、pattern library、field lexicon、prompt、contract type profile 和 evidence selector。当前资产明确只是”代码现状映射”，不声明生产 runtime 已绑定；已有静态 schema 校验但无 runtime loader、发布审批或数据库资产表。本轮不修改业务代码、测试、fixture、expected JSON、OpenAPI、数据库、Docker、workflow、ADR 或 PRD；不进入 `TASK-028` / `TASK-031` / `TASK-032`，不创建 `TASK-033`。
...
- 待确认：`TASK-030` 后续是否继续推进 runtime loader 评审、数据库持久化方案或 model / budget profile 源定义；如进入 runtime loading、发布审批、数据库资产表、模型职责或 EvidenceSlot / CandidateResolver 行为变化，必须重新判断 ADR。
...
5. `TASK_SPEC-030-A` 已完成 Review assets 静态源文件与 manifest 最小建档；`TASK_SPEC-030-B` 已完成 schema 与 manifest 引用校验脚本；`TASK_SPEC-030-C` 已完成版本治理与变更流程最小建档；`TASK-030` 后续仍只有 runtime loader 评审和 model / budget profile 源定义等候选建议，不代表已授权创建或派发下一实现任务。

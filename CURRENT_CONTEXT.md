...
3. 后续实际推进方向合并为 `TASK-DATA-001` / MVP E2E 人工 anchor 准备；TABLE_CELL 必须先取得独立人工 anchor 标注。
4. `TASK-GOV-006` 已通过 PR #18 完成云端 PR 触发验证；该结果仍不得用于进入业务实现或配置 required status checks。
5. `TASK-033` 已归档；后续正式 MVP 端到端验收、`TASK-DATA-001` 人工 anchor 标注、required status checks 或 `TASK-028` / `TASK-031` / `TASK-032` 均需另行定界或人工确认。

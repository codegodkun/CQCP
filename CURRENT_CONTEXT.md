...
- `TASK-036` 的 A 已合并，B1 已提交为 `137c202`，B2 已提交推送为 `e18f7a4`。`TASK_SPEC-036-C1` inactive runtime core 已通过最终独立实现审计 `GO` 和 Codex Review Intake 接纳，并形成可重建提交 `de20de8`：C1 六类 `188/188`、相关四类回归 `223/223`、B1 Node `100/100`、validator `9/9`、`bootJar` 通过；完整 Gradle `472 tests / 1 environment failure`，唯一失败为 PostgreSQL/Flyway `contextLoads()` hostname 依赖。C1 尚未 push；`v20260715.1` 仍未接入 execution，C2 与正式 MVP E2E 均未解锁。
...
- `TASK_SPEC-035-A` 已实现、接纳并提交；candidate comparison 路线不再阻塞。`TASK-036-C1` 已接纳并提交，但仅为 inactive runtime core；C2 execution activation 尚未创建、冻结、审计或授权，因此 `TASK-034` 正式重跑继续被阻塞。不得提前改写 v1 的 9 `MATCH` / 18 `MISMATCH` 与 57/57 `NOT_OBSERVABLE` 证据。
...
- 已确认：`TASK_SPEC-036-B2` 十路径实现与治理写回已精确提交为 `e18f7a4` 并推送；C1 inactive runtime core 已通过独立实现审计与 Codex Review Intake 接纳并提交为 `de20de8`，当前尚未 push。C2 仍须由用户另行授权创建、冻结和独立审计。
...
4. `TASK_SPEC-036-C1` 已接纳并提交为 `de20de8`。下一步由用户决定是否授权 post-commit Memory Writeback 提交及 push；C2 仍须另行授权创建与冻结。当前不运行正式 E2E，不自动进入 C2。

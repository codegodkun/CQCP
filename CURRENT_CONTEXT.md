...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备，三份合同共 63 条逐出处明细已由 `ZK` 接受。`TASK_SPEC-DATA-001-A` test-only revision 已解除首轮四项测试契约缺口；Codex 重跑定向测试为 10/10 与 4/4，独立 agent delta 审计 `GO`，无 blocking finding。PR #28 已于 2026-07-13 合并，merge commit 为 `23c66aaed34326f242f9fb395d784518421f1575`；`TASK_SPEC-DATA-001-A` 已完成并迁移到 `tasks/done/`，父任务进入 `READY_FOR_INDEPENDENT_PRE_ARCHIVE_AUDIT`。`TASK-EVAL-001` 不归档，不进入 `TASK-028` / `TASK-031` / `TASK-032`。
...
- `DEBT-001-05` 真实 DOCX `TABLE_CELL` 已形成 `ACCEPTED_HUMAN_GROUND_TRUTH` 的 `BLOCK` / `TABLE_CELL` 逐出处明细，并已转换为 fixture、expected JSON 引用和定向测试；但完整 MVP E2E 尚未执行，不得宣称真实 DOCX 自动化端到端覆盖已验证。
...
- 已确认：`TASK-DATA-001` 选定的 `CQCP-MVP-DOCX-001` / `002` / `003` 可用于项目验证，脱敏合规，data owner / 确认人为 `ZK`；人工标注使用 XLSX；三份合同 63 条逐出处明细已提升为 `ACCEPTED_HUMAN_GROUND_TRUTH`；`TASK_SPEC-DATA-001-A` 实现已通过 PR #28 合并。待确认：父任务归档前独立只读审计结论及 Codex Review Intake Decision。
...
3. `TASK-DATA-001` 已完成规则冻结、63 条 ground truth 和转换实现；PR #28 已合并。下一步为父任务归档前独立只读审计与 Codex 单独 Review Intake，不得跳过审计直接归档。
4. `TASK-GOV-006` 已通过 PR #18 完成云端 PR 触发验证；该结果仍不得用于进入业务实现或配置 required status checks。
5. `TASK-033` 已归档；`TASK-DATA-001` 人工 anchor 标注及 fixture / expected JSON 引用 / 定向测试转换已经完成。正式 MVP 端到端验收仍需另行定界；required status checks 或 `TASK-028` / `TASK-031` / `TASK-032` 均需遵守各自任务门禁。

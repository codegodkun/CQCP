...
- `TASK-GOV-004 Phase 3：GitHub Actions CI minimal setup` 已按用户授权启动并新增 `.github/workflows/ci.yml`：backend job 运行 Java 21 + Gradle 8.10.2 + PostgreSQL 16 service + `gradle test`，admin-web job 运行 Node.js 24 + `npm ci` + lint/test/build。PR #5 首轮 backend job 暴露 runner 默认 Gradle 9.6.0 与当前测试运行时不兼容，已改为显式使用 Gradle 8.10.2；该 workflow 仍待新一轮 GitHub Actions 验证，不得写成 CI PASS、required checks 已生效或 `PR_REQUIRED_CHECKS`。

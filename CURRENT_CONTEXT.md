...
- `TASK-036` 的 `TASK_SPEC-036-A` 未激活 carrier foundation 已随 PR #32 合并。`TASK_SPEC-036-B1` 已通过提交 `137c202` 形成可重建静态 policy 基线。`TASK_SPEC-036-B2` runtime loader / fail-closed activation gate foundation 已通过 Codex Review、最终独立实现审计 `GO`、loader/gate 209/209 与真实 Linux Docker image build，并通过 commit `e18f7a4` 推送到远端同名分支。新 `v20260715.1` 仍为 `DRAFT / NOT_BOUND / false/false/NONE`，现有任务继续走 legacy single anchor；C 与正式 MVP E2E 均未解锁。
...
- `TASK_SPEC-035-A` 已实现、接纳并提交；candidate comparison 路线不再阻塞。`TASK-036-A` 未激活 carrier 已接纳，B1 与 B2 均已提交，B2 仍仅为 loader/gate foundation；C 的真实 scope collector 与可靠异值裁判尚无冻结规格，因此 `TASK-034` 正式重跑继续被阻塞；不得提前改写 v1 的 9 `MATCH` / 18 `MISMATCH` 与 57/57 `NOT_OBSERVABLE` 证据。
...
- 已确认：`TASK_SPEC-036-B2` 十路径实现与治理写回已精确提交为 `e18f7a4` 并推送；C 仍须另行创建、冻结和独立审计。
...
4. `TASK_SPEC-036-B1` 已提交为 `137c202`；`TASK_SPEC-036-B2` 已提交并推送为 `e18f7a4`。下一步候选为另行创建和冻结 C；当前不运行正式 E2E，也不自动进入 C 实现。

...
- 2026-07-09 用户确认 `TASK-EVAL-001` 独立只读审计已完成；Codex Review Intake Decision 为 `NO-GO TO ARCHIVE / KEEP ACTIVE`。DoD #12 固定未通过、不可补足；`TASK-GOV-005` 不补足 DoD #12。该审计结论针对 `TASK-EVAL-001` 当时的 parser-backed expected / anchor；此后 `TASK-DATA-001` 已形成 63 条独立人工 ground truth，尚未完成 fixture / expected JSON / 测试转换和完整 MVP E2E 验证。`TASK-028` / `TASK-031` / `TASK-032` 继续 `NO-GO`。
...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备。`CQCP-MVP-DOCX-001` / `002` / `003` 共 63 条逐出处明细已由 `ZK` 接受为 `MANUAL_DOCX_REVIEW / ACCEPTED_HUMAN_GROUND_TRUTH`；63 条比较结果均为 `MATCH`，其中 6 条合同标题/前言名称明确排除。三份样本均确认可用、脱敏合规，data owner 为 `ZK`。用户直接修正 DOCX 003 的合同金额大写、签约通知表标段和《阳光合作协议》前言标段；Codex 未修改 DOCX，该用户侧修正已按任务边界例外记录并拟纳入同一可重建基线。2026-07-13 提交前独立只读审计在两轮阻塞修正后最终给出 `GO`，允许范围为当前 7 个精确路径。转换规格 `TASK_SPEC-DATA-001-A` 已冻结；首次编码前基线核对仍记录为 `NO-GO / EXECUTION BASELINE NOT READY / PLAN REQUEST NOT DISPATCHED`，需在提交和隔离环境准备后重新判定。尚未授权实现；不归档 `TASK-EVAL-001`，不进入 `TASK-028` / `TASK-031` / `TASK-032`。
...
- `DEBT-001-05` 真实 DOCX `TABLE_CELL` 已形成 `ACCEPTED_HUMAN_GROUND_TRUTH` 的 `BLOCK` / `TABLE_CELL` 逐出处明细；但 fixture / expected JSON / 测试尚未转换，不得宣称自动化覆盖或完整 MVP E2E 已验证。
...
- 已确认：`TASK-DATA-001` 选定的 `CQCP-MVP-DOCX-001` / `002` / `003` 可用于项目验证，脱敏合规，data owner / 确认人为 `ZK`；人工标注使用 XLSX；三份合同 63 条逐出处明细已提升为 `ACCEPTED_HUMAN_GROUND_TRUTH`；后续由 `TASK_SPEC-DATA-001-A` 承接 fixture / expected JSON / 测试转换。待确认：编码前规格映射计划及 Codex 实现放行决定。
...
3. `TASK-DATA-001` 已完成规则冻结、3 份候选选择和 63 条 `ACCEPTED_HUMAN_GROUND_TRUTH`；`TASK_SPEC-DATA-001-A` 已冻结但基线门禁为 `NO-GO`。下一步先完成当前成果提交前审查并形成包含已接受 XLSX 的可重建 Git 基线，再准备干净隔离执行环境；此前不派发编码前规格映射计划。
4. `TASK-GOV-006` 已通过 PR #18 完成云端 PR 触发验证；该结果仍不得用于进入业务实现或配置 required status checks。
5. `TASK-033` 已归档；`TASK-DATA-001` 人工 anchor 标注已经完成。后续 fixture / expected JSON / 测试转换、正式 MVP 端到端验收、required status checks 或 `TASK-028` / `TASK-031` / `TASK-032` 均需遵守各自任务门禁或另行定界。

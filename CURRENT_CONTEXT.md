...
- `TASK-030` 本轮已完成 Review assets 版本化治理只读盘点，文件：`tasks/active/TASK-030-review-assets-versioning-governance.md`。结论：`packages/review-assets/` 目前只有目录骨架和 `.gitkeep`，真实规则 / 正则 / EvidenceSlot runtime / CandidateResolver / deterministic rule 仍主要位于后端 Java 代码；execution、snapshot、TuningPacket 和 ModelCallIntent 已有版本引用字段，但尚无资产源文件、manifest、schema 校验、runtime loader、发布审批或数据库资产表。本轮仅更新任务文件、项目记忆和任务地图，不修改业务代码、测试、fixture、expected JSON、OpenAPI、数据库、Docker、workflow、ADR 或 PRD；不进入 `TASK-028` / `TASK-031` / `TASK-032`，不创建 `TASK-033`。
...
- 待确认：`TASK-030` 后续是否先按仓库内静态资产源文件推进 `TASK_SPEC-030-A/B`，还是先评审 runtime loader / 数据库持久化方案；如进入 runtime loading、发布审批、数据库资产表、模型职责或 EvidenceSlot / CandidateResolver 行为变化，必须重新判断 ADR。
...
5. `TASK-030` 已给出 Review assets 静态源文件、schema 校验、版本引用对齐、Pattern / field lexicon 映射、runtime loader 评审和 model / budget profile 源定义的后续切分建议；这些只是候选建议，不代表已授权创建或派发实现任务。

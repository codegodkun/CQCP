...
- `TASK-036` 的 A 已合并，B1 已提交为 `137c202`，B2 已提交推送为 `e18f7a4`。`TASK_SPEC-036-C1` inactive runtime core 已通过最终独立实现审计 `GO` 和 Codex Review Intake 接纳：C1 六类 `188/188`、相关四类回归 `223/223`、B1 Node `100/100`、validator `9/9`、`bootJar` 通过；完整 Gradle `472 tests / 1 environment failure`，唯一失败为 PostgreSQL/Flyway `contextLoads()` hostname 依赖。C1 当前等待精确 commit 授权；`v20260715.1` 仍未接入 execution，C2 与正式 MVP E2E 均未解锁。
...
- `TASK_SPEC-035-A` 已实现、接纳并提交；candidate comparison 路线不再阻塞。`TASK-036-C1` 已接纳但尚未 commit，且仅为 inactive runtime core；C2 execution activation 尚未创建、冻结、审计或授权，因此 `TASK-034` 正式重跑继续被阻塞。不得提前改写 v1 的 9 `MATCH` / 18 `MISMATCH` 与 57/57 `NOT_OBSERVABLE` 证据。
- `TASK-036-A/B1/B2/C1` 均不构成 production execution 激活；当前生产调用仍不得传入 `ready=true`，`v20260715.1` 仍不是已生效 execution 版本。
...
- 不进入 C2 production execution activation、正式 DOCX coverage 验收或 RuleSetVersion 生产启用；C1 仅接纳 inactive runtime core。
...
- 已确认：`TASK_SPEC-036-B2` 十路径实现与治理写回已精确提交为 `e18f7a4` 并推送；C1 inactive runtime core 已通过独立实现审计与 Codex Review Intake 接纳，当前尚未 commit。C2 仍须在 C1 可重建 commit 后由用户另行授权创建、冻结和独立审计。
...
4. `TASK_SPEC-036-C1` 已接纳，下一步是用户决定是否授权精确 stage/commit；形成可重建 commit 后，才可另行授权创建与冻结 `TASK_SPEC-036-C2`。当前不运行正式 E2E，不自动进入 C2。

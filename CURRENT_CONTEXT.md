...
1. 后续如需处理 `DEBT-001-03` parser provenance 或 `DEBT-001-05` TABLE_CELL，应另行定界任务；TABLE_CELL 必须先取得独立人工 anchor 标注。
2. 继续禁止 `TASK-028` / `TASK-031` / `TASK-032` 抢跑。
3. `TASK-EVAL-001` 仍为 active / 暂停归档；DoD #12 与历史授权治理债务未解决前，不得恢复归档。

...
const V29_RATIO_SEMANTIC_EXCLUSIONS = ["INACTIVE_PAYMENT_BRANCH"];
const RATIO_REVIEW_POINT_CODES = new Set([
  "PREPAYMENT_RATIO_CONSISTENCY",
  "PROGRESS_PAYMENT_RATIO_CONSISTENCY",
  "COMPLETION_PAYMENT_RATIO_CONSISTENCY",
  "SETTLEMENT_PAYMENT_RATIO_CONSISTENCY",
...
const POLICY_SCOPE_VERSIONS = new Map([
  ["v20260715.1", "consistency-scope-v20260715.1"],
  ["v20260729.1", "consistency-scope-v20260729.1"],
]);
...
function expectedSemanticExclusions(assetVersion, reviewPointCode) {
  if (reviewPointCode === "PARTY_A_NAME_CONSISTENCY") {
    return PARTY_A_SEMANTIC_EXCLUSIONS;
  }
  if (assetVersion === "v20260729.1" && RATIO_REVIEW_POINT_CODES.has(reviewPointCode)) {
    return V29_RATIO_SEMANTIC_EXCLUSIONS;
  }
  return [];
}
...
for (const a of parsedAssets) {
  ...
  for (const rp of rps) {
    ...
    if (!sp || typeof sp !== "object") {
      ...
      const expectedScopeVersion = POLICY_SCOPE_VERSIONS.get(a.data.version);
      if (!expectedScopeVersion) {
        errors.push(`${a.filePath}: ${code}: unsupported policy asset version ${JSON.stringify(a.data.version)}`);
      } else if (sp.version !== expectedScopeVersion) {
        errors.push(`${a.filePath}: ${code}: scopePolicy.version must be ${JSON.stringify(expectedScopeVersion)}, got ${JSON.stringify(sp.version)}`);
      }
      ...
      // Per-version, per-point semantic exclusion
      const expectedSemantic = expectedSemanticExclusions(a.data.version, code);
      if (!arraysEqual(sp.strongExcludedSemanticContexts, expectedSemantic)) {
        errors.push(`${a.filePath}: ${code}: scopePolicy.strongExcludedSemanticContexts must be ${JSON.stringify(expectedSemantic)} for ${a.data.version}, got ${JSON.stringify(sp.strongExcludedSemanticContexts)}`);
      }
    }
...
// v29 is a full immutable snapshot of v15 with a small allowlisted delta.
const v15ReviewPoints = parsedAssets.find((a) =>
  a.data.assetId === "cqcp.review-points.mvp.consistency-set.v20260715.1");
const v29ReviewPoints = parsedAssets.find((a) =>
  a.data.assetId === "cqcp.review-points.mvp.consistency-set.v20260729.1");
if (v15ReviewPoints && v29ReviewPoints) {
  const normalized = deepClone(v29ReviewPoints.data);
  normalized.assetId = v15ReviewPoints.data.assetId;
  normalized.version = v15ReviewPoints.data.version;
  normalized.createdAt = v15ReviewPoints.data.createdAt;
  normalized.changeReason = v15ReviewPoints.data.changeReason;
  normalized.source.paths[2] = v15ReviewPoints.data.source.paths[2];
  normalized.source.bindingNote = v15ReviewPoints.data.source.bindingNote;
  for (const point of normalized.reviewPoints) {
    point.consistencyPolicy.scopePolicy.version = "consistency-scope-v20260715.1";
    if (RATIO_REVIEW_POINT_CODES.has(point.reviewPointCode)) {
      point.consistencyPolicy.scopePolicy.strongExcludedSemanticContexts = [];
    }
  }
  if (JSON.stringify(normalized) !== JSON.stringify(v15ReviewPoints.data)) {
    errors.push(`${v29ReviewPoints.filePath}: v29 review-point snapshot contains fields outside the allowlisted v15 delta`);
  }
}
...
const v15RuleSet = parsedAssets.find((a) =>
  a.data.assetId === "cqcp.ruleset.mvp.consistency-set.v20260715.1");
const v29RuleSet = parsedAssets.find((a) =>
  a.data.assetId === "cqcp.ruleset.mvp.consistency-set.v20260729.1");
if (v15RuleSet && v29RuleSet) {
  const normalized = deepClone(v29RuleSet.data);
  normalized.assetId = v15RuleSet.data.assetId;
  normalized.version = v15RuleSet.data.version;
  normalized.createdAt = v15RuleSet.data.createdAt;
  normalized.changeReason = v15RuleSet.data.changeReason;
  normalized.source.paths[2] = v15RuleSet.data.source.paths[2];
  normalized.source.bindingNote = v15RuleSet.data.source.bindingNote;
  normalized.moduleVersions.reviewPointDefinitions =
    deepClone(v15RuleSet.data.moduleVersions.reviewPointDefinitions);
  normalized.runtimePolicy.notes = deepClone(v15RuleSet.data.runtimePolicy.notes);
  if (JSON.stringify(normalized) !== JSON.stringify(v15RuleSet.data)) {
    errors.push(`${v29RuleSet.filePath}: v29 RuleSet snapshot contains fields outside the allowlisted v15 delta`);
  }
}

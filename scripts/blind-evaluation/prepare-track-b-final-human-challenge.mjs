...
assert.match(
  nonce ?? "",
  /^TBF1-[a-f0-9]{32}$/,
  "challenge nonce must be an opaque TBF1 identifier"
);
...
const outputRoot = "outputs/task-eval-004/track-b-final-v1";
...
assert.equal(manifest.connectivityStatus, "CONNECTIVITY_GATE_GO");
assert.equal(manifest.connectivityFormalAdmissionAffected, false);

const challenge = {
  schemaVersion: "task-eval-004-track-b-final-human-challenge-v1",
  status: "AWAITING_EXPLICIT_HUMAN_DECISIONS",
  nonce,
  createdAt,
  expiresAt,
  connectivityEvidencePath: manifest.connectivityEvidencePath,
  ...
  instruction:
    ...
    "确认前不得创建 model input 或发起正式 admission 调用。"
};
...
process.stdout.write(
  `${JSON.stringify({
    ...
    nonce,
    connectivityEvidenceSha256:
      challenge.connectivityEvidenceSha256,

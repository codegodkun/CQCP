...
public class DocxWordParserSpike {
    ...
    public WordParserSpikeDocument parse(Path docxPath) throws IOException {
        try (InputStream inputStream = Files.newInputStream(docxPath);
            ...
            return parseDocument(docxPath.getFileName().toString(), document);
        }
    }

    public WordParserSpikeDocument parse(
            byte[] docxBytes,
            String sourceFileId) throws IOException {
        try (InputStream inputStream = new ByteArrayInputStream(docxBytes);
             XWPFDocument document = new XWPFDocument(inputStream)) {
            return parseDocument(sourceFileId, document);
        }
    }

    private WordParserSpikeDocument parseDocument(String sourceFileId, XWPFDocument document) {

...
final class ConsistencyCandidateCollector {
    ...
    private static final Set<String> SUPPORTED_SCOPE_VERSIONS = Set.of(
            "consistency-scope-v20260715.1",
            "consistency-scope-v20260729.1");

    private final ProbeDescriptor descriptor;
    private final List<SemanticClassifier> semanticClassifiers;

    ConsistencyCandidateCollector(
            ProbeDescriptor descriptor,
            List<SemanticClassifier> semanticClassifiers) {
        this.descriptor = Objects.requireNonNull(descriptor, "descriptor");
        this.semanticClassifiers = List.copyOf(semanticClassifiers);
    }
    ...
    ConsistencyCandidateBatch collect(
        ...
        if (!SUPPORTED_SCOPE_VERSIONS.contains(policy.scopeVersion())) {
        ...
        for (WordParserSpikeDocument.DocumentBlock block : allBlocks) {
            ...
            // 4. Versioned semantic exclusion
            boolean semanticEnabled = !policy.strongExcludedSemanticContexts().isEmpty()
                    && ("consistency-scope-v20260729.1".equals(policy.scopeVersion())
                        || reviewPointCode == ReviewPointCode.PARTY_A_NAME_CONSISTENCY);
            if (semanticEnabled) {
    ...
    private SemanticResult checkSemanticExclusion(
        ...
        for (var sc : semanticClassifiers) {

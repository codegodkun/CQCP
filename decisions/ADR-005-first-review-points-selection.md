...
第一轮 MVP 首批审核点采用以下最小清单，状态仍待人工确认：
...
| `AMOUNT_TAX` | `TAX_AMOUNT_FORMULA_CONSISTENCY` | 否 | 后端公式裁判，结构化税额字段或合同内税额 slot 不足时不生成业务 finding |
...
- EvidenceSlot：
  ...
  - `taxRateCandidate`：optional，先作为 raw ratio candidate / diagnostic candidate 处理；税率是否新增固定 CandidateRole 仍待确认，MVP 不为此临时扩展 role。
...
- `TAX_AMOUNT_FORMULA_CONSISTENCY` 用于验证后端公式裁判，但不作为 core，避免税额字段不稳定时拖低 MVP 闭环。
...
- 后续样本验证至少需要覆盖主体、金额、税额和五个月度付款比例的正例、冲突例、缺证据例。
- `TAX_RATE` 是否新增为固定 CandidateRole 仍待确认；MVP 不因该点临时扩展 role。
...
- 当前尚未编码，无数据迁移影响。
- 若人工确认后删减或调整首批审核点，更新本 ADR 或创建替代 ADR，并同步任务包、`ROADMAP.md`、`docs/ai-review.md`。

...
- 能构建候选索引和审核点计划。
- 能用规则、正则、本地 Gemma/A30 局部辅助和后端确定性裁判形成结果。
...
| 8 | 税率 | `taxRate` | 税额公式诊断；是否新增固定 CandidateRole 待确认 |
...
- 当前仍不建议开始 scaffold；应先完成关键 ADR 人工确认、首批 ReviewPointDefinition 草稿、最小 OpenAPI 契约、Word parser baseline plan 和 model gateway baseline。
...
当前仍不建议开始 scaffold；应优先完成 `TASK-004-word-parser-baseline-plan`、`TASK-005-model-gateway-and-budget-baseline`、首批 `ReviewPointDefinition` 草案任务、最小 OpenAPI 契约任务，并人工确认关键 ADR。

...
- 至少 30 份为由业务方/data owner 确认可用于验证的非合成合同样本、批准历史案例或隔离评测环境中的授权原始合同。
...
- MVP 验收样本由业务方提供合成中文 `.docx` 工程采购合同，或提供由业务方/data owner 确认可用于项目验证的中文 `.docx` 工程采购合同。平台不负责自动识别样本是否已脱敏，也不在 MVP 中提供脱敏检测或自动脱敏能力；样本进入仓库或评测环境的前提，是业务方/data owner 已确认其可用于项目验证。

...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备；`001` / `002` / `003` 共 63 条逐出处明细已接受，可重建基线和隔离 worktree 已准备。2026-07-13 `TASK_SPEC-DATA-001-A` 修订版计划复审为 `NO-GO / CODING-PLAN REVISION ROUND 2 REQUIRED / NO IMPLEMENTATION AUTHORIZATION`；剩余直接字段/派生字段分离与空字符串保真两项，不解除 `TASK-EVAL-001` / `TASK-028` 门禁
...
| `TASK-DATA-001` | MVP E2E 人工 anchor 准备 | Data / Evaluation | Active / 人工 anchor 准备完成 / 修订版计划复审 NO-GO / 待第二次修订 | 文件：`tasks/active/TASK-DATA-001-mvp-e2e-human-anchor-preparation.md`；63 条逐出处明细均已接受；转换规格：`tasks/active/TASK_SPEC-DATA-001-A-human-anchor-fixture-expected-test-conversion.md`；剩余两项见 §10.1，未授权实现，不进入 `TASK-028` / `TASK-031` / `TASK-032` |
...
6. `TASK-DATA-001` 已完成规则冻结、63 条 `ACCEPTED_HUMAN_GROUND_TRUTH`、可重建提交和隔离 worktree；`TASK_SPEC-DATA-001-A` 修订版计划复审仍 `NO-GO`，下一步仅修订剩余两项并重审，Codex 明确 `GO` 前不得转换。

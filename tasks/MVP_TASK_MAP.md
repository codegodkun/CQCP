...
- `TASK-036` 已完成多出处一致性证据架构冻结；A 已合并，B1 已提交为 `137c202`，B2 已提交推送为 `e18f7a4`。C1 inactive runtime core 已通过最终独立实现审计 `GO`、Codex Review Intake、C1 六类 `188/188` 与相关回归 `223/223`，当前等待精确 commit 授权。C2 execution activation 仍锁定，普通任务仍为 legacy single anchor
...
| `TASK-036` | 多出处一致性证据架构冻结与分批实现治理 | A / Architecture | Active / A Merged / B1 Committed / B2 Pushed / C1 Accepted Awaiting Commit / C2 Locked | 文件：`tasks/active/TASK-036-multi-occurrence-consistency-evidence-architecture-freeze.md`；C1 文件：`tasks/active/TASK_SPEC-036-C1-consistency-set-runtime-core.md`；现有规则集仍未激活 |
...
- 实现拆分：A 为未激活 carrier foundation；B1/B2 承接静态 policy、loader 与 fail-closed gate；C1 承接未接入 execution 的真实 collector、readiness 与 point-local verdict core；C2 承接 execution binding 与原子 activation。
...
- 当前门禁：A 已合并，B1/B2 已提交，C1 inactive runtime core 已接纳但尚未 commit。C2 仅可在 C1 形成可重建 commit 且用户另行授权后创建、冻结与审计；不得据 C1 接纳宣称 57/57 正式 coverage、激活生产多 anchor 或重跑正式 E2E。
- B1 结果：`v20260715.1`、九点 `CONSISTENCY_SET`、`maxCandidates=8`、`occurrenceBudget=64`、scope/canonicalization/anchor identity 与未激活校验已实现并提交；Codex 最终复验 100/100、validator 9/9，第二轮独立实现审计 `GO_TO_ACCEPT`。新资产仍为 `DRAFT / NOT_BOUND / false/false/NONE`。
- B2 结果：首轮实现审查与独立审计 findings 已全部定点关闭；loader/gate 209/209、Node 100/100、validator 9/9、boot JAR 两 entry 与真实 Linux Docker build 均通过，最终独立实现审计 `GO`。B2 不接线 execution/snapshot，C 前 `v20260715.1` 固定 fail-closed。
- C1 结果：最终独立实现审计 `GO / NO_BLOCKING_FINDINGS`；C1 六类 `188/188`、相关四类回归 `223/223`、Node `100/100`、validator `9/9` 与 `bootJar` 通过。完整 Gradle 472 项仅 `contextLoads()` 因 PostgreSQL/Flyway hostname 环境依赖失败。Codex Review Intake 已接纳 C1 inactive runtime core；等待精确 commit 授权。
...
9. `ADR-016` 已接受，`docs/ARCHITECTURE.md` v0.10 已同步并审计 `GO`；TASK_SPEC-036-A 未激活 carrier 已随 PR #32 合并，B1 已提交为 `137c202`，B2 loader/gate foundation 已提交推送为 `e18f7a4`。下一步候选为另行冻结 C；正式 E2E 仍需单独授权。

...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备；`TASK_SPEC-DATA-001-A` test-only revision 已解除四项测试契约缺口，Codex 定向测试 10/10 与 4/4，独立 agent delta 审计 `GO`。PR #28 三项 checks 已通过并合并，merge commit 为 `23c66aaed34326f242f9fb395d784518421f1575`；父任务进入归档前独立审计准备，不解除 `TASK-EVAL-001` / `TASK-028` 门禁
...
| `TASK-DATA-001` | MVP E2E 人工 anchor 准备 | Data / Evaluation | Active / READY_FOR_INDEPENDENT_PRE_ARCHIVE_AUDIT / PR #28 merged | 文件：`tasks/active/TASK-DATA-001-mvp-e2e-human-anchor-preparation.md`；63 条逐出处明细均已接受；转换规格已完成并归档到 `tasks/done/TASK_SPEC-DATA-001-A-human-anchor-fixture-expected-test-conversion.md`；PR #28 merge commit `23c66aaed34326f242f9fb395d784518421f1575`；不得跳过独立审计直接归档，不进入 `TASK-028` / `TASK-031` / `TASK-032` |
...
6. `TASK-DATA-001` 已完成规则冻结、63 条 `ACCEPTED_HUMAN_GROUND_TRUTH` 和转换实现；PR #28 已合并。下一步为父任务归档前独立只读审计与 Codex Review Intake。
...
10. 后续如需处理 parser provenance，必须另行定界任务；real DOCX `TABLE_CELL` 独立人工 anchor 已完成 fixture / expected JSON 引用 / 定向测试转换，但该结果不等于完整 MVP E2E 已验证。

...
|---|---|---|---|
| `TASK-MVP-002` | L3 | `91d5e16…` Formal R7 PASS；完整 verification 在 D1 实际 452 与固定 451 门禁处停止，verification/freeze 452 同步定向 `11/11` PASS，待新 clean candidate/全量验证/冻结/三审 | execution 清单、精确结果、受控 preview/下载、左右工作台 |
...
| `TASK-034` | L3 | `91d5e16…` Formal R7 恢复 57 MATCHED + 6 EXCLUDED 并 sealed；因后续 D1 计数脚本变化而 superseded，待新 clean-HEAD Formal R7 | 27/27 point PASS、57 MATCHED + 6 human EXCLUDED 的目标门禁不变 |
| `TASK-036` | L3 | B/C/D seam 进入 Core；D1 当前 452/452，verification/freeze 计数同步定向 `11/11` PASS，待新 R7/verification/freeze/三审与主线 merge | deterministic eligibility、FamilyModelCallPlan、runtime EvidencePacket |
...
| `TASK-034` | MVP E2E 人工 anchor 正式验收执行 | A / Evaluation | Active / 91d5e16 Formal R7 PASS and Sealed / Superseded by D1 Count Script Change / Fresh Formal R7 Pending | 文件：`tasks/active/TASK-034-mvp-e2e-human-anchor-acceptance-execution.md`；`91d5e16…` 已恢复 57 `MATCHED` + 6 `EXCLUDED`，但完整 verification 因 D1 452 与固定 451 门禁停止；test-only resolver 红绿与 harness `19/19` 保持通过；正式目标仍为 3 份样本 27/27 candidate `MATCH`、27/27 `PointStatus=PASS`、57 `MATCHED` + 6 human `EXCLUDED`、70 production semantic exclusions、0 SYS/Finding；不声明 Production Ready，不进入 `TASK-028` / `TASK-031` / `TASK-032` |
...
| `TASK-036` | 多出处一致性证据架构冻结与分批实现治理 | A / Architecture | Active / A Merged / B1-B2-C1-C2-D1-D2 In PR #37 / D1 452 Gate Synced / Fresh Verification Pending | 文件：`tasks/active/TASK-036-multi-occurrence-consistency-evidence-architecture-freeze.md`；A 随 PR #32 合并；B1/B2/C1/C2/D1/D2 已提交并 push 至 PR #37，但尚未 merge；`91d5e16…` D1 实际 452 全通过，verification/freeze 计数门禁已限定同步，需从新 clean HEAD 完成 R7、完整 verification、freeze 与三审 |

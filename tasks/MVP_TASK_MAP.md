...
- `TASK-037 / ADR-017 / TASK_SPEC-037-A` 已从 PR #33 后的新主线独立冻结并完成实现；规格、架构同步与第二轮实现审计均为 `GO`，ADR-017 Accepted，Codex 实现 Review Intake 为 `ACCEPT_IMPLEMENTATION / GO_TO_COMMIT_PR`；当前待独立 L3 PR/CI/merge
...
| `TASK-037` | Execution Binding Release 与 Profile Seed | L3 高风险治理 | Active / ADR-017 Accepted / Implementation Audit GO / Codex ACCEPT / PR Pending | 文件：`tasks/active/TASK-037-execution-binding-release-and-profile-seed.md`；独立 L3 PR，不激活 TASK-036-C2 |
...
12. `FEATURE-MVP-001` 基于 `401fd05`；`TASK_SPEC-MVP-001-A` 已实现、通过 Codex Review Intake 与独立只读复审并提交为 `f8d76e7`，使用 legacy `v20260705.1`。`TASK_SPEC-MVP-001-B` 已通过 Codex Review Intake 与独立只读实现审计并提交为 `21203998da7482e25d86136c14d1f42cff2d2ec7`，未 push。`TASK_SPEC-MVP-001-C` 已通过 Codex Review Intake，Docker C 定向 43/43、backend 全量 313/313，并提交为 `0eb9d9e312c6698fb4532e623362bea091165f87`，未 push。`TASK_SPEC-MVP-001-D` 已通过 Codex Review Intake 并提交为 `9fcf8574bcefe8884ec2cd8ec93cb71a8c5fbecb`，未 push。`TASK_SPEC-MVP-001-E` 已冻结状态页、轮询、安全结果跳转与正式结果路由；下一门仅允许执行方提交 E 编码前规格映射，尚未授权 E 编码。

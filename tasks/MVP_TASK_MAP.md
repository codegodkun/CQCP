...
| TASK-021 | Result URL 查询接口最小实现 | 提供最小结果查询路径，支持 `taskId + executionId` 查询正式 snapshot，并遵守 `ADR-002` 的公开可见性边界 | P0 | A | 是 | 主导公开接口边界、结果可见字段、错误码和 URL 选择规则 | 可在接口签名冻结后补 controller/service/test | 否 | TASK | 依赖 `TASK-019`、`TASK-020`、`TASK-014`、`ADR-002`、`ADR-003` | 已建档 |

...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备；`001` / `002` / `003` 共 63 条逐出处明细已接受，可重建基线和隔离 worktree 已准备。2026-07-13 `TASK_SPEC-DATA-001-A` 第二修订版计划获 `GO / CODING-PLAN ACCEPTED WITH BINDING CLARIFICATION / IMPLEMENTATION AUTHORIZED WITHIN §0.3 / NO COMMIT OR PUSH`；不解除 `TASK-EVAL-001` / `TASK-028` 门禁
...
| `TASK-DATA-001` | MVP E2E 人工 anchor 准备 | Data / Evaluation | Active / 人工 anchor 准备完成 / 编码前计划 GO / §0.3 实现已授权 | 文件：`tasks/active/TASK-DATA-001-mvp-e2e-human-anchor-preparation.md`；63 条逐出处明细均已接受；转换规格：`tasks/active/TASK_SPEC-DATA-001-A-human-anchor-fixture-expected-test-conversion.md`；实现仅限 §0.3，禁止 commit / push，不进入 `TASK-028` / `TASK-031` / `TASK-032` |
...
6. `TASK-DATA-001` 已完成规则冻结、63 条 `ACCEPTED_HUMAN_GROUND_TRUTH`、可重建提交和隔离 worktree；`TASK_SPEC-DATA-001-A` 编码前计划已获 GO，下一步仅执行 §0.3 并提交实现报告，禁止 commit / push。

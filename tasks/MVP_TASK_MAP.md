...
- `TASK-036` 已完成多出处一致性证据架构冻结；`TASK_SPEC-036-A` 未激活 carrier foundation 已随 PR #32 合并，`TASK_SPEC-036-B1` 已提交为 `137c202`，`TASK_SPEC-036-B2` loader / fail-closed activation gate foundation 已通过 Codex Review、最终独立实现审计 `GO`、209/209 定向测试与真实 Linux Docker build，并提交推送为 `e18f7a4`。C 真实 collector 与裁判尚未冻结，普通任务仍为 legacy single anchor
...
| `TASK-036` | 多出处一致性证据架构冻结与分批实现治理 | A / Architecture | Active / A Merged / B1 Committed / B2 Committed and Pushed / C Pending | 文件：`tasks/active/TASK-036-multi-occurrence-consistency-evidence-architecture-freeze.md`；B1 提交 `137c202`；B2 提交 `e18f7a4`；现有规则集仍未激活 |
...
- 当前门禁：A 已随 PR #32 合并，B1 静态不可变策略已提交为 `137c202`，B2 loader / fail-closed gate foundation 已提交推送为 `e18f7a4`。下一步候选为另行创建和冻结 C；不得据 A/B1/B2 foundation 宣称 57/57 coverage、激活生产多 anchor 或重跑正式 E2E。

...
| `TASK-EVAL-006` | L3 | R1-R4 模型评测 GO；R5/R6 失效；R7 verification PASS，subject `47d84299…` 的三审因 CC package isolation `P0=1 / blocking=1` 失败 | human seal `0f23b9eb…` 早于模型访问；Codex/DeepSeek 六维 9/9、controls 3/3；R7 verification `1805feb1…` / console `8aba6687…` / Node 56/56 / Java 1/1（5 executed）均通过；但完整 diff 在 CC 包内再暴露 hash-only excluded ground truth/comparison，CC GO 作废、另一 Codex audit 中断；`providerAdmissionEstablished=false`，A0/A1/A2 继续阻塞 |

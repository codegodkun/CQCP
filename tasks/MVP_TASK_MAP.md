...
基线：`origin/master@1035739b751386176e47c6871738a62bff86de02`

当前先收口 `MILESTONE-MVP-002-CORE`：

| 任务 | Task Level | 当前状态 | Core 边界 |
|---|---|---|---|
| `TASK-MVP-002` | L3 | `97b55a0…` 已关闭 `8d01dd9a…` 的 P1/P2；新 verification 在 TASK-034 test-only public anchor resolver fail closed，单文件红绿与 harness `19/19` 已通过，待新 clean candidate/全量验证/冻结/三审 | execution 清单、精确结果、受控 preview/下载、左右工作台 |
| `TASK-MODEL-001` | L3 | 实现与补证保留；`3bc24a84…` 因后续 tracked 修复失效，待同一新 HEAD 重建 | immutable Model Profile、Secret Reference、allowlist、readiness/connectivity |
| `TASK-EVAL-002` | L3 | Track A 完成；旧 run-v3 为 `NO_GO_MODEL_MISMATCH`；历史 CRLF seal 保留修复 Windows/Linux 31/31 且 unblind 恢复；新 holdout 未开始 | Track A、Track B EvidencePacket、严格模型意见与解盲 |
| `TASK-034` | L3 | 历史 v29/R7 PASS；`97b55a0…` verification 暴露 test-only resolver 未优先消费 cell preview ref，单文件红绿与 harness `19/19` PASS，待 clean-HEAD Formal R7 | 27/27 point PASS、57 MATCHED + 6 human EXCLUDED 的目标门禁不变 |
| `TASK-036` | L3 | B/C/D seam 进入 Core；`8d01dd9a…` 的 parser cell span / 公共 anchor findings 已在 working tree 限定修复，待新 R7/verification/freeze/三审与主线 merge | deterministic eligibility、FamilyModelCallPlan、runtime EvidencePacket |

Core source diff 明确排除 Provider A0、standing/CC 审计传输、A1/A2/A3 与
`outputs/**`。派生 R7、M3、browser、Compose 证据由冻结清单按 SHA-256 引用。
`ea19a52a…`、`33890cb…`、`81e47f…`、`5bcfe410…` 与 `3bc24a84…` 的旧 freeze/verification
均已失效；任何新源码变化后只接受重新生成且
绑定新 HEAD/tree 的证据和三份全新审计报告。

完成条件：

- Core-only 完整验证、bootJar、admin-web、OpenAPI、Compose/browser 全部通过；
- CC AUDIT 与两个全新 Codex auditor 绑定同一 subject，全部
  `GO / P0=P1=P2=blocking=0`；
- CI 通过并按用户既有授权完成 merge。

Core merge 后才建立模型未见的 12 packet Track B holdout（9 eligible + 3 controls）；
人工答案先封印、只运行一次、失败即停止。Provider A0/A1/A2 的恢复不属于本 Core
integration unit；A3 已拆出本 Milestone。
## 当前结论
...
- `TASK-036` 的 A 已随 PR #32 合并；B1/B2/C1/C2/D1/D2 已提交并进入
  PR #37，形成 R7 与 Track B zero-call 证据；PR #37 尚未 merge。后续 subject
  `8d01dd9a…` 因 TABLE_ROW capture span 与公共 SourceAnchor locationLevel findings
  失效；限定修复已完成定向验证，仍需从新 clean HEAD 重建 R7、完整 verification、
  freeze 与三审
...
| `TASK-034` | MVP E2E 人工 anchor 正式验收执行 | A / Evaluation | Active / Historical v29 Formal R7 PASS / Public Anchor Harness Fix Targeted PASS / Fresh Formal R7 Pending | 文件：`tasks/active/TASK-034-mvp-e2e-human-anchor-acceptance-execution.md`；`97b55a0…` verification 的失败 R7 已封存；test-only resolver 现优先消费 parser-issued cell preview ref，红测 `1/1 FAIL`、绿测 `1/1 PASS`、harness `19/19 PASS`；正式目标仍为 3 份样本 27/27 candidate `MATCH`、27/27 `PointStatus=PASS`、57 `MATCHED` + 6 human `EXCLUDED`、70 production semantic exclusions、0 SYS/Finding；不声明 Production Ready，不进入 `TASK-028` / `TASK-031` / `TASK-032` |
...
| `TASK-036` | 多出处一致性证据架构冻结与分批实现治理 | A / Architecture | Active / A Merged / B1-B2-C1-C2-D1-D2 In PR #37 / 8d01dd9a Findings Remediated / Fresh Verification Pending | 文件：`tasks/active/TASK-036-multi-occurrence-consistency-evidence-architecture-freeze.md`；A 随 PR #32 合并；B1/B2/C1/C2/D1/D2 已提交并 push 至 PR #37，但尚未 merge；`8d01dd9a…` 的 TABLE_ROW capture span 与公共 anchor findings 已限定修复，需从新 clean HEAD 完成 R7、完整 verification、freeze 与三审 |
...
- 当前实现与门禁：A 已随 PR #32 合并；B1/B2/C1/C2/D1/D2 已在
  `codex/task-mvp-002` worktree 按冻结 TASK_SPEC 集成，D1 定向 443
  assertions、D2 seam 20 assertions、v29/R7 与 Track B Codex zero-call
  工件已形成并进入 PR #37，但 PR 尚未 merge。旧 subject 已因 tracked 字节修复失效，新的完整
  verification、freeze 和三审仍待完成；不得据此声明 Production Ready、Provider admission 或解锁
...
9. `ADR-016` 已接受，`docs/ARCHITECTURE.md` v0.10 已同步并审计 `GO`；TASK_SPEC-036-A 已随 PR #32 合并。B1/B2/C1/C2/D1/D2 已提交并 push 至 PR #37，但尚未 merge；冻结 45568f… 的统一验证已因最终审计 `NO_GO` 失效，后续 subject `3bc24a84…` 又因 tracked 字节修复失效，完整 verification、freeze 和三审须从零重建。

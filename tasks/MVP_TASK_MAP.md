...
12. `FEATURE-MVP-001` 基于 `401fd05`；`TASK_SPEC-MVP-001-A` 已实现、通过 Codex Review Intake 与独立只读复审并提交为 `f8d76e7`，使用 legacy `v20260705.1`。`TASK_SPEC-MVP-001-B` 已通过 Codex Review Intake 与独立只读实现审计并提交为 `21203998da7482e25d86136c14d1f42cff2d2ec7`，未 push。`TASK_SPEC-MVP-001-C` 已冻结 execution 状态查询 API；下一门仅允许执行方提交 C 编码前规格映射，尚未授权 C 编码。

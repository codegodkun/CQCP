...
- `TASK-036` 已完成多出处一致性证据架构冻结；`TASK_SPEC-036-A` 未激活 carrier foundation 已随 PR #32 合并，`TASK_SPEC-036-B1` 已提交为 `137c202`。`TASK_SPEC-036-B2` loader / fail-closed activation gate foundation 已实现并通过 Codex Review、最终独立实现审计 `GO`、209/209 定向测试与真实 Linux Docker build，当前等待精确 commit；C 真实 collector 与裁判尚未冻结，普通任务仍为 legacy single anchor
...
| `TASK-036` | 多出处一致性证据架构冻结与分批实现治理 | A / Architecture | Active / A Merged / B1 Committed / B2 Accepted Awaiting Commit / C Pending | 文件：`tasks/active/TASK-036-multi-occurrence-consistency-evidence-architecture-freeze.md`；B1 提交 `137c202`；B2 loader/gate foundation 已通过独立审计与真实 Linux Docker build；现有规则集仍未激活 |
...
- 当前门禁：A 已随 PR #32 合并，B1 静态不可变策略已提交为 `137c202`，B2 loader / fail-closed gate foundation 已接纳但等待精确 commit。B2 提交后才可另行冻结 C；不得据 A/B1/B2 foundation 宣称 57/57 coverage、激活生产多 anchor 或重跑正式 E2E。
...
9. `ADR-016` 已接受，`docs/ARCHITECTURE.md` v0.10 已同步并审计 `GO`；TASK_SPEC-036-A 未激活 carrier 已随 PR #32 合并，B1 已提交为 `137c202`，B2 loader/gate foundation 已接纳并等待精确 commit。下一步顺序为 B2 commit → 另行冻结 C；正式 E2E 仍需单独授权。

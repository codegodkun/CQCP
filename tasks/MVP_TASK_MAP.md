...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备；`TASK_SPEC-DATA-001-A` 首轮实现的 8 个路径均在范围内，当前数据产物正确且两个定向测试通过，但测试契约仍缺少类型、顺序、全列、精确 schema 和完整 positive-case 冻结断言。Codex Decision 为 `NO-GO / TEST CONTRACT REVISION REQUIRED / DATA ARTIFACTS RETAINED / NO COMMIT OR PUSH`；不解除 `TASK-EVAL-001` / `TASK-028` 门禁
...
| `TASK-DATA-001` | MVP E2E 人工 anchor 准备 | Data / Evaluation | Active / 数据产物可保留 / 首轮实现 Review NO-GO / 仅测试返工 | 文件：`tasks/active/TASK-DATA-001-mvp-e2e-human-anchor-preparation.md`；63 条逐出处明细均已接受；转换规格：`tasks/active/TASK_SPEC-DATA-001-A-human-anchor-fixture-expected-test-conversion.md`；仅允许修改新测试补齐四项契约断言，禁止 commit / push，不进入 `TASK-028` / `TASK-031` / `TASK-032` |
...
6. `TASK-DATA-001` 已完成规则冻结和 63 条 `ACCEPTED_HUMAN_GROUND_TRUTH`；`TASK_SPEC-DATA-001-A` 下一步仅修订新测试的四项契约断言并重审，其他实现路径不得修改，禁止 commit / push。

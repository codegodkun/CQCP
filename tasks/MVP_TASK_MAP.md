...
- `TASK-DATA-001` 已完成真实 DOCX 人工 anchor 准备；`001` / `002` / `003` 共 63 条逐出处明细已由 `ZK` 接受为 `ACCEPTED_HUMAN_GROUND_TRUTH`。可重建基线和隔离 worktree 已准备。2026-07-13 `TASK_SPEC-DATA-001-A` 首版编码前计划经 Codex 审查为 `NO-GO / CODING-PLAN REVISION REQUIRED / NO IMPLEMENTATION AUTHORIZATION`；不构成代码、测试、fixture、expected JSON 或 parser 实现授权，不解除 `TASK-EVAL-001` / `TASK-028` 门禁
...
| `TASK-DATA-001` | MVP E2E 人工 anchor 准备 | Data / Evaluation | Active / 人工 anchor 准备完成 / 编码前计划审查 NO-GO / 待修订 | 文件：`tasks/active/TASK-DATA-001-mvp-e2e-human-anchor-preparation.md`；63 条逐出处明细均已接受；转换规格：`tasks/active/TASK_SPEC-DATA-001-A-human-anchor-fixture-expected-test-conversion.md`；首版计划需按 §10.1 六项要求修订，未授权实现，不进入 `TASK-028` / `TASK-031` / `TASK-032` |
...
6. `TASK-DATA-001` 已完成规则冻结、63 条 `ACCEPTED_HUMAN_GROUND_TRUTH`、可重建提交和隔离 worktree；`TASK_SPEC-DATA-001-A` 首版编码前计划审查 `NO-GO`，下一步仅修订并重审计划，Codex 明确 `GO` 前不得转换。

...
- `FEATURE-MVP-001` 基于 `401fd05`；A~F+F2 已接纳，真实浏览器 Demo 为 `SUCCESS / 9 PASS / FULL_REVIEWED`，F1 测试隔离修复提交为 `439a020`；F2 已关闭首轮 findings，最终 v5 冻结覆盖 102 个路径。CC AUDIT 与两个 Codex 独立 subagent 均为 `GO`，P0/P1/P2 与 blocking findings 均为 0；父任务及 A~F/F1/F2 已归档，当前待一个 Feature PR、CI 与 merge，尚未 push 或创建 PR
...
- `TASK-035` test-only `mvp-e2e-candidate-comparison-v2` 与 `TASK_SPEC-035-A` 已实现并随 PR #32 合并；正式 E2E 未重跑
- `TASK-036` 的 A 已随 PR #32 合并；B1/B2/C1 只存在于远端分支 `codex/task-036-consistency-set-runtime@3adcab4`，未进入主线；C2 未实现，普通主线任务仍为 legacy 单 anchor
...
| `TASK-MVP-001` | 合同审核用户闭环 Demo | L2 Feature（风险触发型） | Done / DUAL_AUDIT_GO / ARCHIVED / FEATURE_PR_PENDING | 文件：`tasks/done/TASK-MVP-001-contract-review-user-loop-demo.md`；Feature 基线 `401fd05`；A~E commits `f8d76e7 / 2120399 / 0eb9d9e / 9fcf857 / 539c9e6`，F1 `439a020`；v5 冻结覆盖 102 个路径，CC AUDIT 与两个 Codex 独立 subagent 均为 GO、blocking findings=0；真实浏览器、PostgreSQL 与原始运行证据位于 `outputs/task-mvp-001-demo-acceptance/`；保持 legacy `v20260705.1` 与 `MVP_DEMO_MOCK`，不运行 TASK-034，不声明 Production Ready |
...
| `TASK-036` | 多出处一致性证据架构冻结与分批实现治理 | A / Architecture | Active / A Merged / B1-B2-C1 Remote Branch Only / C2 Not Implemented | 文件：`tasks/active/TASK-036-multi-occurrence-consistency-evidence-architecture-freeze.md`；A 随 PR #32 合并；B1/B2/C1 在 `codex/task-036-consistency-set-runtime@3adcab4`，未进入主线；现有主线仍未激活新 RuleSetVersion |
...
12. `FEATURE-MVP-001` 基于 `401fd05`；A~E 已接纳并形成 commits `f8d76e7 / 2120399 / 0eb9d9e / 9fcf857 / 539c9e6`。F 已完成 Compose、真实浏览器、PostgreSQL 和上传持久化验收，结果为 `SUCCESS / 9 PASS / FULL_REVIEWED`；F1 测试隔离修复提交为 `439a020`。F2 关闭首轮审计 findings 后，admin-web 63/63、后端连续两轮 313/313、Compose 重建和真实状态→结果路径均通过；最终 v5 冻结的 CC AUDIT 与两个 Codex 独立 subagent 均为 GO、blocking findings=0。父任务及 A~F/F1/F2 已归档；下一门是一个 Feature PR、CI 与 merge。

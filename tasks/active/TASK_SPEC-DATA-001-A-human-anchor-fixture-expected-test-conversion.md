...
> **版本**：v0.1
> **状态**：IMPLEMENTATION ACCEPTED / COMMITTED AND PUSHED / PR #28 CHECKS PENDING / MERGE NOT AUTHORIZED
> **创建日期**：2026-07-12
> **起草**：CODEX
...
状态：test-only revision 已回传 / Codex 与独立 agent 复核 GO / 实现已提交并 push / PR #28 已创建并等待 checks / 未授权合并。
...
- 新建 3 份 `packages/test-fixtures/human-anchors/CQCP-MVP-DOCX-00{1,2,3}.json`。
- 修改 3 份 `packages/test-fixtures/expected/CQCP-MVP-DOCX-00{1,2,3}.json`。
- 修改 `packages/test-fixtures/README.md`。
- 新建 `apps/api-server/src/test/java/com/cqcp/apiserver/reviewengine/HumanAnchorGroundTruthFixtureTest.java`。
- 共 8 个路径，均在 §0.3 允许范围内；实现未 commit、未 push。

### 10.3 实现摘要与字段映射

- Codex 独立核对当前三份 fixture：22/19/22 共 63 条，57 条纳入、6 条排除；16 个直接字段当前均为 JSON string，18 个 occurrence key 齐全，顺序与 XLSX 一致。
- 三份 expected JSON 仅新增 `goldenExpected.humanAnchorGroundTruth`；既有 `positiveCases`、`expectedCanonicalAnchors[]` 和其他 sibling 未变。
- 独立 agent 对路径范围、人工来源独立性和当前 canonical baseline 给出 `GO`，无 blocking finding。
- 当前阻塞不是 fixture 数据错误，而是新测试未完整锁定已冻结契约。

### 10.4 验收标准逐项自检

未接受。当前实现产物内容通过抽查，但测试契约存在 §11 四项 blocking findings，尚不能进入提交前接纳。

### 10.5 测试命令与原始摘要

- `HumanAnchorGroundTruthFixtureTest`：Codex 重跑，`10 tests / 0 failures / 0 errors`，`BUILD SUCCESSFUL`。
- `ParserBackedEvidenceOverlapBaselineTest`：Codex 重跑，`4 tests / 0 failures / 0 errors`，`BUILD SUCCESSFUL`。
- `git diff --check`：通过。
- 测试通过不解除 §11 的断言有效性缺口。
...
- 远端分支：`origin/codex/task-data-001-a-human-anchor-conversion`；push 成功；PR #28 已创建，目标分支为 `master`，checks pending，未授权合并。

---

## 11. Codex Review Intake

当前 Decision：`GO / IMPLEMENTATION ACCEPTED / INDEPENDENT READ-ONLY AUDIT GO / COMMITTED AND PUSHED / PR #28 CHECKS PENDING / MERGE NOT AUTHORIZED`。

2026-07-13 首轮实现 Review Intake blocking findings：

1. 16 个直接字段通过 `JsonNode.asText()` 后比较，无法证明 JSON 节点类型是 string；`occurrenceNo` 先转成 Java `String` 再做 `isInstanceOf(String.class)` 是恒真断言。必须对每个直接字段先断言 `isTextual()`，并对 `includedInConsistencyEvaluation` 断言 `isBoolean()`。
2. XLSX 与 fixture 通过 `occurrenceNo` 建索引后比较，未断言 occurrence 数组顺序与 XLSX 行顺序一致。必须逐索引比较 `occurrenceNo`，使单纯重排 fixture 能导致测试失败。
3. XLSX 表头存在性只检查 7 列，未覆盖全部 16 个直接字段。必须把 16 个直接字段与 `sampleId`、`notes`、`docxPath` 全部纳入 required-column STOP 检查。
4. `shouldNotContainParserDerivedFields` 只证明实际 key 是白名单子集，未直接断言每条 occurrence 恰好包含 18 个冻结 key；`parserBackedCanonicalAnchorsRemainFrozen` 也未按获批计划冻结 `positiveCases` 数量、`caseId`、`reviewPointCode`、`expectedCandidateValue`。必须补齐精确 key set 与完整 positive-case 断言。

非阻塞已确认事实：当前三份 fixture 内容、顺序和类型均正确；两个定向测试通过；独立 agent 对范围、独立性和当前 baseline 给出 `GO`。

2026-07-13 test-only revision 再审：

- 16 个直接字段均先断言 `JsonNode.isTextual()`；`includedInConsistencyEvaluation` 断言 `isBoolean()`。
- fixture occurrence 已按数组索引与 XLSX 行顺序逐项比较。
- required-column 门禁覆盖 16 个直接字段及 `sampleId`、`notes`、`docxPath` 共 19 列。
...
4. 已完成：用户明确授权后，Codex 精确 stage、commit 并 push 8 个实现路径；PR #28 已创建并等待 checks，未授权合并。
...
- 执行者不得修改长期项目记忆。
- 当前实现、push 和 PR 创建已完成；下一步仅等待 PR #28 checks 与后续单独合并决定，不是新的实现任务，不得继续修改实现文件。

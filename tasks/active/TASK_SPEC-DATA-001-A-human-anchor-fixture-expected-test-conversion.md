...
2. 严格执行 §2、§3、§8 和 §9；
3. 在回复中按 §10.2-§10.6 格式输出实现报告并停止，等待 Codex 审查；不得编辑本 TASK_SPEC 或其他 `tasks/` 文件。
...
1. Claude Code / DeepSeek 仅按 §0.3 和本次绑定澄清执行实现，在回复中按 §10.2-§10.6 格式输出实现报告后停止；不得编辑 `tasks/`、commit 或 push。
...
- 执行者不得修改长期项目记忆。
- 当前下一步是 Claude Code / DeepSeek 在隔离 worktree 内执行 §0.3 实现并在回复中提交实现报告；授权不扩展到任何禁止路径、commit 或 push。

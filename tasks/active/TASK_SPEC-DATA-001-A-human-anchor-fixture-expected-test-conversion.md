...
> **版本**：v0.1
> **状态**：IMPLEMENTATION REVIEW NO-GO / TEST CONTRACT REVISION REQUIRED / NO COMMIT OR PUSH
> **创建日期**：2026-07-12
> **起草**：CODEX
...
Codex 已完成首轮实现审查并给出 test-only revision。当前只允许：
1. 修改 `HumanAnchorGroundTruthFixtureTest.java`，补齐 §11 列出的四项可证伪断言；
2. 不得修改当前三份 fixture、001-003 expected JSON、fixture README 或其他任何文件；
3. 重跑 §9 两个定向测试，在回复中输出修订说明、原始测试摘要和四项 git 验证后停止；不得编辑本 TASK_SPEC 或其他 `tasks/` 文件。
...
状态：首轮实现已回传 / Codex Review Intake NO-GO / 仅测试返工。
...
当前 Decision：`NO-GO / TEST CONTRACT REVISION REQUIRED / DATA ARTIFACTS RETAINED / NO COMMIT OR PUSH`。
...
1. Claude Code / DeepSeek 只修改新建的 `HumanAnchorGroundTruthFixtureTest.java`，完成上述四项 test-only revision；不得修改其他 7 个当前实现路径。
2. 修订完成后，Codex 重新审查测试 diff、两个定向测试输出和完整范围。
3. 必要时派独立 agent 只读复核。
...
- 执行者不得修改长期项目记忆。
- 当前下一步是 Claude Code / DeepSeek 在隔离 worktree 内执行 §11 四项 test-only revision；不得修改数据产物、任务文件、commit 或 push。

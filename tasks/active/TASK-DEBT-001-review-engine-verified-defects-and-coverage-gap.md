...
状态：Active（Step 1 标准记录与非 Git Review Intake 已完成；Step 3 第一批 TASK_SPEC-DEBT-001-A 已实现并经 Codex 审查、提交前独立只读审计通过，已 commit / push 为 `3223d6760a977fe9deaf722e63b50bcbb6ce3611`；2026-06-27 post-push 独立只读复审为 GO；第二批 TASK_SPEC-DEBT-001-B 已冻结到编码前规格映射计划阶段，未派发实现）
...
第一批 `TASK_SPEC-DEBT-001-A` 已完成 Codex 审查、提交前独立只读审计、commit 与 push，且 2026-06-27 post-push 独立只读复审为 `GO`。后续第二批规格为 `tasks/active/TASK_SPEC-DEBT-001-B-collect-pattern-candidates-value-format-signal.md`，用于定界 `collectPatternCandidates()` 的 `valueFormatSignal` 修复；当前已冻结到编码前规格映射计划阶段，但尚未批准 B 批次实现。
...
- 当前只允许派发 B 批次编码前规格映射计划；不得把 A 批次完成或 B 规格冻结扩展为 B 批次实现授权。Codex 审查 B 编码前规格映射计划 `GO` 前，不得授权 CC/DeepSeek 修改代码或测试。
- `TASK-EVAL-001-B` 已事后条件接纳；不提交新的 B 代码、测试、fixture 或 expected JSON 变更，`TASK-EVAL-001` 归档以及 `TASK-028 / TASK-031 / TASK-032` 门禁保持不变。

## Next Task Handoff

`TASK_SPEC-DEBT-001-A` 已完成实现审查、提交前独立只读审计、commit 与 push；提交为 `3223d6760a977fe9deaf722e63b50bcbb6ce3611`。2026-06-27 GitHub 云端 post-push 独立只读复审为 `GO`。`TASK-DEBT-001` 父任务仍为 active，`TASK_SPEC-DEBT-001-B collectPatternCandidates valueFormatSignal 修复` 已冻结到编码前规格映射计划阶段；当前只允许派发映射计划，不得直接实现。`TASK-028` / `TASK-031` / `TASK-032` 门禁继续保持。

...
状态：Active（Step 1 标准记录与非 Git Review Intake 已完成；Step 3 第一批 TASK_SPEC-DEBT-001-A 已实现并经 Codex 审查、独立只读审计通过；未 commit / 未 push）
...
v3 整改计划要求先把已确认问题写入仓库内可追踪记录，再决定后续分批 TASK_SPEC、人工标注和独立审计。本任务承载“4 条代码缺陷 + 1 条覆盖盲区”的标准记录、后续分流线索和持续门禁。2026-06-26 起，第一批 `TASK_SPEC-DEBT-001-A` 已作为角色分离试点完成实现与审查；父任务仍保持 active，其他问题不得顺手修复。
...
第一批局部修复 TASK_SPEC 为 `tasks/active/TASK_SPEC-DEBT-001-A-resolve-text-evidence-signals.md`。2026-06-26 状态：Claude Code / DeepSeek 已完成授权范围内实现；Codex Review Intake Decision 为 `A. 可以合并`；独立只读审计结论为 `GO`；当前仍未 stage、未 commit、未 push。
...
第一批 `TASK_SPEC-DEBT-001-A` 已完成 Codex 审查和独立只读审计。后续第二批应通过新的冻结 TASK_SPEC 定界，明确与 A 批次三信号计算原则的一致点、差异点及差异理由；当前尚未创建或批准 B 批次实现。
...
- 【2026-06-26 更新】`TASK_SPEC-DEBT-001-A` 已完成编码前映射计划、Codex 放行、Claude Code / DeepSeek 实现、Codex 实现审查和独立只读审计；Codex Review Intake Decision 为 `A. 可以合并`，只读审计结论为 `GO`。
- 当前只允许对 A 批次进行提交前判断；不得把 A 批次完成扩展为 B 批次或其他分流实现授权。
- `TASK-EVAL-001-B` 已事后条件接纳；不提交新的 B 代码、测试、fixture 或 expected JSON 变更，`TASK-EVAL-001` 归档以及 `TASK-028 / TASK-031 / TASK-032` 门禁保持不变。

## Next Task Handoff

`TASK_SPEC-DEBT-001-A` 已完成实现审查与独立只读审计，下一步如需提交必须先取得用户明确 commit 授权；如需 push，必须再次取得用户明确 push 授权。`TASK-DEBT-001` 父任务仍为 active，后续候选为 `TASK_SPEC-DEBT-001-B collectPatternCandidates valueFormatSignal 修复` 规格起草，但当前尚未创建、冻结或派发，不得直接实现。`TASK-028` / `TASK-031` / `TASK-032` 门禁继续保持。

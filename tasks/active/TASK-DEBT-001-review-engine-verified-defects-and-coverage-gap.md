...
状态：Active（Step 1 标准记录与非 Git Review Intake 已完成；Step 3 第一批 TASK_SPEC 草案已创建并进入审查；未批准实现）
...
第一批局部修复 TASK_SPEC 已起草为 `tasks/active/TASK_SPEC-DEBT-001-A-resolve-text-evidence-signals.md`，当前状态为 `Ready for Review（未派发实现）`。该状态只表示 Codex 已按用户授权进入 Step 3 规格起草；不代表 Claude Code / DeepSeek 已获准实现，也不代表修复工作已开始。
...
- 后续任何实现必须先经过冻结 TASK_SPEC、编码前规格映射计划、Codex 放行和实现后审查门禁；当前仅允许审查 `TASK_SPEC-DEBT-001-A`，不得派发 Claude Code / DeepSeek 实现。
...
- `TASK_SPEC-DEBT-001-A` 仍需用户 / Codex 审查；未完成审查前不得粘贴执行指令块给 Claude Code / DeepSeek。
- 不修改业务代码、测试、fixture 或 expected JSON；不派发 Claude Code / DeepSeek。
- `TASK-EVAL-001-B` 已事后条件接纳；不提交新的 B 代码、测试、fixture 或 expected JSON 变更，`TASK-EVAL-001` 归档以及 `TASK-028 / TASK-031 / TASK-032` 门禁保持不变。

## Next Task Handoff

Step 2 `CURRENT_CONTEXT.md` 逐条认领审计、B 事后独立复核和定向测试复跑已有独立 agent 报告摘要，但本任务文件不以自身证明这些事项完成；父任务归档前仍需独立 agent 复核原始报告、console 输出、commit、diff 和对应证据。当前已进入 Step 3 规格起草，下一步是审查 `TASK_SPEC-DEBT-001-A`，仍不得派发 Claude Code / DeepSeek 实现，直到 Codex 明确接受规格并放行执行。

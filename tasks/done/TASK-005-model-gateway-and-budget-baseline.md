...
- 相关 ADR：`ADR-002-v1-result-and-diagnostic-contract` 草案、`ADR-006-model-profile-switching-and-public-provider-scope`
...
- 明确 `STANDARD` budget 的启动边界或待确认方式
- 明确模型失败和预算不足的降级行为
...
- 是否需要更新 `docs/ai-review.md`、`docs/deployment.md` 或 `docs/database.md`：是，已更新 `docs/ai-review.md`、`docs/deployment.md`
- 是否需要新增或更新 ADR：否，本次冻结沿用既有架构和 ADR 结论，仅收口 MVP 基线文档
...
- 完成日期：2026-06-10
- 变更文件：`docs/model-gateway-budget-baseline.md`、`docs/ai-review.md`、`docs/deployment.md`、`CURRENT_CONTEXT.md`、`changelog/2026-06.md`、`tasks/done/TASK-005-model-gateway-and-budget-baseline.md`
- 测试结果：未运行代码测试；已完成文档一致性核对，确认 ModelGateway、ModelCallIntent、GemmaExtractionArtifact、timeout/retry/circuit breaker 和预算降级边界已明确
- 遗留问题：具体 token 数值仍待样例集与实测验证后确认
- 备注：本次只冻结预算结构与降级策略，不凭空写死 token 数值，符合“待确认”写作规则

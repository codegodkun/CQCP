...
- 规则/正则/Gemma 混合路由。
- 本地模型生产辅助（默认通过本地 OpenAI-compatible 模型服务接入，当前目标硬件为 A30 24GB）。
...
冻结基线来自 `TASK-001-v1-mvp-scope-gate`、`TASK-002-entry-order-decision`、`TASK-003-first-review-points-selection`、`TASK-007-first-review-point-definitions-draft`、`TASK-008-architecture-prd-definition-index-alignment` 和 `TASK-009-mvp-scope-freeze`。后续任务不得把 Pilot / Production Readiness 能力提前纳入第一轮 MVP；如需改变冻结范围，必须通过新任务和必要 ADR 推进。
技术栈冻结已由 `TASK-010-tech-stack-freeze` 落地，并以 `ADR-010-technology-stack-freeze.md` 作为长期决策来源。

...
`requiresHigherBudget / recommendedBudgetProfile` 不进入第一轮 MVP 普通结果页；外部系统和普通结果页只看到业务化 `NOT_CONCLUDED` 原因和人工核对提示。预算诊断保留在管理台任务详情、评测报告和 AI 调优包中。

MVP 首批结构化字段 key 采用 PRD 冻结最小契约。外部 API 枚举编码固定为：

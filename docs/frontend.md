...
- 存在 `requiresHigherBudget=true` 时，显示当前审核深度下未完成可靠判断，并在管理台展示 `recommendedBudgetProfile`。
...
当 `requiresHigherBudget=true` 且 caller policy 允许 correction execution 时，结果页可提供“申请深度审核”命令入口。

该入口仍由后端审批 budget profile，不保证自动升级。

同一 task 已存在 non-terminal execution 时，按钮必须禁用并显示“已有审核进行中，请等待完成”。服务端以 `409 TASK_EXECUTION_IN_PROGRESS` 做幂等保护。

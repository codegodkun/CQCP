...
一期 worker 从 PostgreSQL 拉取 `QUEUED` 任务。MVP 数据库正式冻结为 PostgreSQL 15+，迁移工具为 Flyway。
...
## Flyway V2 Execution Binding 基线

ADR-017 新增：

* `review_budget_profile_version`
* `model_profile_config_version`
* `execution_binding_release`

三个表的 identity/version 与 content immutable；`enabled / is_default_for_new_task / readiness_status` 是 lifecycle state。V2 只提供初始 seed 和 resolver 所需约束，不提供发布管理 API。

首次 migration seed：

* `STANDARD / DEEP_REVIEW / EVALUATION` 三类 budget version，比例 `5:1`；只启用 `STANDARD`。
* `MVP_DEMO_MOCK`，限定 `MOCK / DEMO / secretRequired=false / READY`，作为 Demo 唯一 enabled default。
* 一个 `MVP_DEMO / DEMO / ENGINEERING` binding，包含 V1 execution 全部 14 个 `NOT NULL` 字段，以及 `contract_type_profile_code=ENGINEERING_PROCUREMENT` traceability metadata。

binding 的 budget/model reference 使用外键；同一 `purpose + deployment_scope + contract_type_code` 最多一个 `enabled=true` row。`effective_from` 不进入 partial unique index，由 resolver 基于 `Clock` 校验。

发布或回滚必须在单个事务内锁定选择域、撤销旧 enabled/default，再启用新版本。不得删除历史版本或原地修改 content。旧 disabled row 与新 enabled row 可以同时存在。
...
`execution_binding_release` 是部署发布选择记录，不是第四类业务配置包。

Task Creation 选择有效 binding 后，将 14 个字段复制到 execution；历史 execution/snapshot 不回查当前 default。V1 兼容映射固定为 `execution.model_config_version == review_result_snapshot.model_profile_version`。
...
- MVP 首批结构化字段 key 沿用 PRD 冻结最小契约，作为任务输入、数据库和 ReviewResultSnapshot 字段名。
- MVP 枚举编码固定保存为：`pricingMode=FIXED_TOTAL_PRICE / PROVISIONAL_TOTAL_PRICE`，`paymentMethod=MONTHLY / MILESTONE`，`invoiceType=VAT_GENERAL / VAT_SPECIAL`。中文展示文案不作为持久化 code。
...
- 税率新增固定 `CandidateRole=taxRate`，作为 `TAX_AMOUNT_FORMULA_CONSISTENCY` 的 optional EvidenceSlot 候选角色。快照中可保存税率原文候选值、SourceAnchor 和候选归属摘要；税率缺失或不可靠时不执行税额弱校验，不生成无可靠证据的业务 finding。
...
首次 seed 的三类 profile 使用同一个保守启动 `ModelBudget`，只表示结构和版本来源已建立，不代表 DEEP_REVIEW / EVALUATION 已有差异化质量或 SLA。三类 `budgetApprovalPolicyVersion` 为 `budget-approval-policy-mvp-v20260724.1`，release `effectiveFrom / createdAt` 固定为 UTC `2026-07-24T00:00:00Z`。
